...
plot.b <- ggplot(annual, aes(year, anomaly3)) +
  theme_bw() +
  geom_hline(yintercept=c(model.means$max.anomaly3), color=short.list.colors) +
  geom_line() +
  # geom_point() +
  theme(axis.title.x = element_blank()) +
  ylab("SST anomaly") +
...
for(i in 1:nrow(model.means)){
  plot.b <- plot.b + annotate("text", x=2019, y=-1-0.3*i, label=plot.model.names[i], color=short.list.colors[i], adj=1, size=3)
}
