---
title: "EBS SST: observations v. preindustrial envelope"
author: "Mike Litzow"
date: "9/10/2019"
output: pdf_document
---
...
dat <- read.csv("CMIP5 EBS SST.csv")
...
We will compare the preindustrial estimates above with ERSSTv5 observations. The observations are for the same area (54º-64ºN, 180º-160ºW). Note that Walsh et al. (2018) have the latitude written as 51º-64.5ºN, but a different area indicated on their mao. Here is average SST for that area from ERSSTv5:
...
URL <- paste("https://coastwatch.pfeg.noaa.gov/erddap/griddap/nceiErsstv4.nc?sst[(1854-01-01):1:(", year, "-", month, "-01T00:00:00Z)][(0.0):1:(0.0)][(54):1:(64)][(180):1:(200)]", sep="")

download.file(URL, "EBS.box.ersst")

# process
nc <- nc_open("EBS.box.ersst")
...
image(x,y,z, col=tim.colors(64), xlim=c(170,220), ylim=c(48,68))
contour(x, y, z, add=T) 
...
URL <- paste("https://coastwatch.pfeg.noaa.gov/erddap/griddap/nceiErsstv5.nc?sst[(1854-01-01):1:(", year, "-", month, "-01T00:00:00Z)][(0.0):1:(0.0)][(54):1:(64)][(180):1:(200)]", sep="")

download.file(URL, "EBS.box.ersst.latest")

# process
nc <- nc_open("EBS.box.ersst.latest")
